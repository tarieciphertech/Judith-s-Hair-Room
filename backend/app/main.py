...
def create(data:Booking,db:Session=Depends(get_db)):
 ...
 try:db.commit()
  ...
  db.rollback();raise HTTPException(409,detail={'message':'That slot was booked by someone else.','suggestions':suggestions(db,data.date,data.start_time,data.expected_end_time)})
...
def dashboard(db:Session=Depends(get_db)):
 d=date.today();rows=db.scalars(select(Appointment).where(Appointment.appointment_date==d)).all();active=[a for a in rows if a.status!='CANCELLED']
 return {'today':d,'occupied':any(a.status=='IN_PROGRESS' for a in active),'appointments':len(active),'completed':sum(a.status=='COMPLETED' for a in active),'unpaid_deposits':sum(a.payment_status=='UNPAID' for a in active),'revenue':sum(a.agreed_price for a in active if a.status=='COMPLETED')}
